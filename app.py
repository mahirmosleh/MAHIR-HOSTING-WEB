#!/usr/bin/env python3
"""
MAHIR HOSTING - Ultimate VPS Management Panel
...
DEFAULT_CONFIG = {
    ...
    "passwords": {
        "secret": hashlib.sha256("MAHIR HOSTING".encode()).hexdigest(),
...
DEFAULT_USERS = {
    "admin_default": {
        "username": "MAHIR",
        "password_hash": hashlib.sha256("MAHIR HOSTING".encode()).hexdigest(),
        "role": "admin",
...
def _device_cookie_id():
    ...
    return request.cookies.get('MAHIR_device_id') or request.form.get('device_id') or ''
...
app.config['SESSION_COOKIE_NAME'] = 'MAHIR_session'
...
def _mark_device_cookie(trust=False):
    """Login সফল হলে device_id cookie বসায় (client এ MAHIR_device_id কুকি পাঠায়)।
    ...
    did = request.cookies.get('MAHIR_device_id')
    if not did:
...
def log_monitor(server_id, proc_obj):
    ...
    if server_id in SERVERS and SERVERS[server_id].get('process') == proc_obj:
        ...
        SERVERS[server_id]['logs'].append(">>> [MAHIR HOSTING] Process terminated.")
        save_servers()
...
def start_server_internal(server_id, server):
    ...
    try:
        if not server['cmd'] or server['cmd'].strip() == '':
            server['logs'].append(">>> [MAHIR HOSTING] Error: No start command specified")
            return False
        if not os.path.exists(work_dir):
            server['logs'].append(f">>> [MAHIR HOSTING] Error: Directory not found: {work_dir}")
            return False
        ...
        server['logs'].append(f">>> [MAHIR HOSTING] Server started at {time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        server['logs'].append(f">>> [MAHIR HOSTING] Failed to start: {str(e)}")
        return False
            ...
            try:
                if server.get('status') == 'running' and server.get('auto_restart'):
                    ...
                    if current_time - server.get('last_start_time', current_time) >= interval_sec:
                        server['logs'].append(f">>> [MAHIR HOSTING] Auto-restarting...")
    ...
    try:
        ...
        payload = json.dumps({
            "embeds": [{
                ...
                "footer": {"text": "MAHIR HOSTING Panel"},
...
def notify_event(event_type, server_id, message):
    ...
    title = f"{icon} MAHIR HOSTING — {event_type.replace('_',' ').upper()}"
    full_msg = f"{title}\nServer: {server_id}\n{message}\nTime: {time.strftime('%Y-%m-%d %H:%M:%S')}"
...
def create_server():
    try:
        ...
        SERVERS[server_name] = {
            ...
            'logs': [f">>> [MAHIR HOSTING] Server '{server_name}' created at {time.strftime('%Y-%m-%d %H:%M:%S')}"],
...
def upload_server_file():
    try:
        ...
        SERVERS[server_name] = {
            'process': None, 'cmd': start_command, 'cwd': '',
            'logs': [f">>> [MAHIR HOSTING] Server '{server_name}' created with upload at {time.strftime('%Y-%m-%d %H:%M:%S')}"],
            'auto_restart': False, 'restart_interval': '1h', 'last_start_time': 0,
            'status': 'stopped', 'path': server_path,
            'created_at': time.strftime('%Y-%m-%d %H:%M:%S'),
            'notes': notes, 'group': group, 'tags': [], 'env_vars': {}
...
def server_action_api(server_id, action):
    ...
    try:
        if action == 'start':
            ...
            server['process'] = None
            server['status'] = 'stopped'
            server['logs'].append(f">>> [MAHIR HOSTING] Stopped at {time.strftime('%Y-%m-%d %H:%M:%S')}")
            save_servers()
            ...
            server['process'] = None
            server['status'] = 'stopped'
            server['logs'].append(">>> [MAHIR HOSTING] Manual restart triggered...")
            time.sleep(0.5)
            start_server_internal(server_id, server)
            return jsonify({'status': 'ok'})
        elif action == 'delete':
            ...
            SERVERS[new_name] = {
                ...
                'logs': [f">>> [MAHIR HOSTING] Cloned from '{server_id}' at {time.strftime('%Y-%m-%d %H:%M:%S')}"],
                'auto_restart': False, 'restart_interval': '1h', 'last_start_time': 0,
...
def clear_server_logs(server_id):
    ...
    SERVERS[server_id]['logs'] = [f">>> [MAHIR HOSTING] Logs cleared at {time.strftime('%Y-%m-%d %H:%M:%S')}"]
    return jsonify({'status': 'ok'})
...
def install_package(server_id):
    ...
    SERVERS[server_id]['logs'].append(f">>> [MAHIR HOSTING] Installing {pkg_name} via {pkg_type}...")
    def run_install():
        try:
            ...
            if rc != 0 and any('Permission denied' in l or 'error' in l.lower() for l in output_lines):
                SERVERS[server_id]['logs'].append(f">>> [MAHIR HOSTING] Retrying with python3 -m pip...")
            ...
            SERVERS[server_id]['logs'].append(f">>> [MAHIR HOSTING] Installation of {pkg_name} completed.")
        except Exception as e:
            SERVERS[server_id]['logs'].append(f">>> [MAHIR HOSTING] Install error: {str(e)}")
...
def uninstall_package(server_id):
    ...
    SERVERS[server_id]['logs'].append(f">>> [MAHIR HOSTING] Uninstalling {pkg_name}...")
    def run_uninst():
        try:
            process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)
            for line in iter(process.stdout.readline, ''):
                if line: SERVERS[server_id]['logs'].append(line.strip())
            SERVERS[server_id]['logs'].append(f">>> [MAHIR HOSTING] Uninstallation complete.")
        except Exception as e:
            SERVERS[server_id]['logs'].append(f">>> [MAHIR HOSTING] Uninstall error: {str(e)}")
...
# on this or any other MAHIR HOSTING instance by re-uploading the zip.
# =============================================================================
...
def download_full_backup():
    ...
    try:
        manifest = {
            'type': 'MAHIR_HOSTING_FULL_BACKUP',
            'version': 1,
        ...
        zip_buffer.seek(0)
        timestamp = time.strftime('%Y%m%d_%H%M%S')
        download_name = f"MAHIR_HOSTING_FULL_BACKUP_{timestamp}.zip"
...
def restore_full_backup():
    ...
    try:
        ...
        with zipfile.ZipFile(tmp_path, 'r') as zf:
            ...
            if manifest.get('type') != 'MAHIR_HOSTING_FULL_BACKUP':
                return jsonify({'error': 'Invalid backup file: not an MAHIR HOSTING full backup'}), 400
            ...
            for sid, meta in manifest.get('servers', {}).items():
                ...
                SERVERS[sid] = {
                    ...
                    'cmd': meta.get('cmd', ''),
                    'cwd': meta.get('cwd', ''),
                    'logs': [f">>> [MAHIR HOSTING] Restored from full backup at {time.strftime('%Y-%m-%d %H:%M:%S')}"],
...
"""Telegram Bot - Auto Generated by MAHIR HOSTING"""
...
'''
    ...
    SERVERS[bot_name] = {
        ...
        'logs': [f">>> [MAHIR HOSTING] Telegram bot '{bot_name}' created"], 'auto_restart': True,
...
def test_webhook():
    ...
    if wh_type == 'discord':
        send_discord_webhook("🧪 Test Notification", "This is a test message from MAHIR HOSTING Panel. Your Discord webhook is working!", 0x00ff00)
    else:
        send_telegram_webhook("🧪 Test Notification\n\nThis is a test message from MAHIR HOSTING Panel. Your Telegram webhook is working!")
...
JWT_API_KEY = (os.environ.get('JWT_API_KEY') or 'MAHIRfuhx-secret-key').strip()
...
if __name__ == '__main__':
    print("""
    ╔══════════════════════════════════════════════════════════╗
    ║   MAHIR HOSTING v4.0.0 - ADMIN MODE                        ║
    ║   Full access for every logged-in account                ║
    ╚══════════════════════════════════════════════════════════╝
    """)
    print(f"[MAHIR HOSTING] Server started at {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print("[MAHIR HOSTING] Panel running on http://0.0.0.0:5000")
